...
void main() {
  group('LocalDeviceConnectionRepository', () {
    ...
    test(
      'simulateIncomingCall pulses hasIncomingCall for that device',
      () async {
        final repository = LocalDeviceConnectionRepository();
        final events = <DeviceStatus>[];
        final subscription = repository
            .watchStatus('device-1')
            .listen(events.add);
    ...
    test(
      'simulateIncomingCall on one device does not leak into another',
      () async {
        final repository = LocalDeviceConnectionRepository();
        final eventsA = <DeviceStatus>[];
        final eventsB = <DeviceStatus>[];
        final subscriptionA = repository
            .watchStatus('device-a')
            .listen(eventsA.add);
        final subscriptionB = repository
            .watchStatus('device-b')
            .listen(eventsB.add);
        await Future<void>.delayed(Duration.zero);

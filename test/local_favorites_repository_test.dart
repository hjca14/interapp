...
void main() {
  group('LocalFavoritesRepository', () {
    ...
    test('saveAll() then getAll() round-trips favorites for that device', () async {
      final repository = LocalFavoritesRepository();
      const favorites = [
      ...
      expect(reloaded.map((favorite) => favorite.name), ['Portaria', 'Portão']);
    });

    test('favorites are isolated per device', () async {
      final repository = LocalFavoritesRepository();

      await repository.saveAll('device-a', const [Favorite(name: 'A', number: '1')]);
      await repository.saveAll('device-b', const [Favorite(name: 'B', number: '2')]);

...
manual. As Fases 3B.1–3B.5 de Firebase/FCM estão concluídas, com a 3B.5
(registro de instalações) validada ponta a ponta em DEV.
Compartilhamento funcional não foi
...
| FCM / Firebase | 3B.1–3B.5 concluídas e validadas em DEV | Firebase DEV `interbridge-dev`, FCM Android, token inicial/renovação e recebimento foram configurados e validados nas subfases anteriores. O backend de instalações está no interBackend PR #23 e o app integra PUT/DELETE autenticados com UUID local estável; validado ponta a ponta em DEV (`sa-east-1`): login criou exatamente uma `INSTALLATION`/`CLAIM` (`ANDROID`, `FCM`, `com.interbridge.app`, `1.0.0+1`), reinício preservou `installation_id`/`created_at` sem duplicar, logout removeu ambos os itens, e novo login recriou instalação/claim com o mesmo `installation_id`. Sender somente na 3B.6. |
...
| Push notifications | Recebimento (3B.4) e registro de instalação (3B.5) validados em DEV | O recebimento direto da 3B.4 foi validado. O registro/remoção da instalação (`PUT`/`DELETE`) foi validado ponta a ponta em DEV — ver a linha "FCM / Firebase" acima para a evidência sanitizada. Sender FCM (envio real de push a partir do backend) permanece na 3B.6. |

...
manual. As Fases 3B.1–3B.4 de Firebase/FCM estão concluídas; a integração do
app para 3B.5 está implementada, mas aguarda deploy DEV e validação real.
...
| FCM / Firebase | 3B.1–3B.4 concluídas; 3B.5 app implementado | Firebase DEV `interbridge-dev`, FCM Android, token inicial/renovação e recebimento foram configurados e validados nas subfases anteriores. O backend de instalações está no interBackend PR #23 e o app integra PUT/DELETE autenticados com UUID local estável. Deploy DEV e validação ponta a ponta de PUT, renovação e DELETE permanecem pendentes; sender somente na 3B.6. |
...
| Push notifications | Recebimento FCM validado; registro 3B.5 aguardando E2E | O recebimento direto da 3B.4 foi validado. Registro/remoção do app está implementado com fakes, sem sender, alteração de firmware ou afirmação de validação real antes do deploy do backend. |

...
private val RING_CALL_INTENT_KEYS = setOf("v", "event_id", "device_id", "occurred_at")

/**
 * Strictly validates that [payload] matches the minimal `RingCallIntent` v1
 ...
 *
 * Checks structure and types only — exact key set, `v == 1`, `event_id`/
 * `device_id` matching the backend's id formats, and `occurred_at` a
 * parseable UTC instant — not freshness or authorization. It only gates the
 * cosmetic lock-screen presentation ([android.app.Activity.setShowWhenLocked]/
 * [android.app.Activity.setTurnScreenOn]); real authorization and
 * Atender/Dispensar stay entirely inside `IncomingCallPage`.
 *
...
fun isValidRingCallLaunchPayload(payload: String?): Boolean {
    ...
    return try {
        ...
        if (!isVersionOne(json.opt("v"))) return false
